...
namespace FoodHub.Tests.Features.Areas.Commands.UpdateArea
{
    public class UpdateAreaHandlerTests
    {
        ...
        public async Task Handle_ShouldUpdateArea_WhenAreaExists()
        {
            ...
            area.CodePrefix.Should().Be("T1");
            area.Description.Should().Be("Updated");
            area.UpdatedAt.Should().NotBeNull();
            area.UpdatedAt.Should().BeAfter(beforeUpdate);
            _mockUow.Verify(u => u.SaveChangeAsync(It.IsAny<CancellationToken>()), Times.Once);
        }

        private UpdateAreaHandler BuildHandler()
        {
            ...
            var mapConfig = new MapperConfiguration(
                cfg => cfg.CreateMap<Area, GetAreaByIdResponse>(),
                mockLoggerFactory.Object
            );

            return new UpdateAreaHandler(
                _mockUow.Object,
