...
{
    public class CreateInternalReservationCommandValidator
        : AbstractValidator<CreateInternalReservationCommand>
    {
        ...
        )
        {
            ...
            RuleFor(x => x.CustomerName)
                .NotEmpty()
                .WithMessage(_messageService.GetMessage(MessageKeys.Reservation.NameRequired))
                .MaximumLength(100)
                .WithMessage(_messageService.GetMessage(MessageKeys.Reservation.NameMaxLength));

            RuleFor(x => x.CustomerPhone)
                .NotEmpty()
                .WithMessage(_messageService.GetMessage(MessageKeys.Reservation.PhoneRequired))
                .Matches(@"^(0|84|\+84)(3|5|7|8|9)([0-9]{8})$")
                .WithMessage(_messageService.GetMessage(MessageKeys.Reservation.PhoneInvalid));

            RuleFor(x => x.GuestCount)
                .GreaterThan(0)
                .WithMessage(_messageService.GetMessage(MessageKeys.Reservation.InvalidGuestCount));
            ...
            RuleFor(x => x)
                .CustomAsync(
                    async (request, context, cancellationToken) =>
                    {
                        ...
                        {
                            context.AddFailure(
                                _messageService.GetMessage(MessageKeys.Common.DateNotInFuture)
                            );
                            return;
                        }
